...
public class Services {
	...
	private static final String api_version = "1.01A rev.10023";
		  ...
		  try {
			  ...
			  if(dao.itemExists(inObj.getString("hotelId"), inObj.getString("title"))){
				  ...
				  String menuId = dao.addMenuItem(inObj.getString("hotelId"),inObj.getString("title"), inObj.getString("description")
					  , inObj.getString("category"), inObj.getString("station"), inObj.getString("flags"), inObj.getInt("preparationTime"), 
					  inObj.getInt("rate"), inObj.getInt("inhouseRate"), inObj.getInt("costPrice"), inObj.getInt("vegType"), 
					  inObj.getString("image"), inObj.getInt("isTaxable"));
	  ...
	  public String updateMenuItem(String jsonObject) {
		  ...
		  try {
			  ...
			  outObj.put("status",dao.updateMenuItem(inObj.getString("hotelId"), inObj.getString("menuId"), 
					  inObj.getString("title"), inObj.getString("description"), inObj.getString("category"), 
					  inObj.getString("station"), inObj.getString("flags"), inObj.getInt("preparationTime"), 
					  inObj.getInt("rate"), inObj.getInt("inhouseRate"), inObj.getInt("costPrice"), 
	  ...
	  public String changeState(String jsonObject) {
		  ...
		  try {
			  inObj = new JSONObject(jsonObject);
			  outObj.put("status",dao.changeMenuItemState(inObj.getString("hotelId"), inObj.getString("menuId"), inObj.getInt("state")));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getEmployeeByDesignation(@QueryParam("hotelId") String hotelId, @QueryParam("designation") String deisgnation) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray outArr = new JSONArray();
		  ...
		  try {
			  ...
			  for(int x=1; x<=2; x++) {
				  ...
				  for(int i=0; i<attendance.size(); i++) {
					  ...
					  obj.put("checkInTime", attendance.get(i).getCheckInTimeHHMM() == null ? "" : attendance.get(i).getCheckInTimeHHMM());
					  obj.put("checkOutTime", attendance.get(i).getCheckOutTimeHHMM() == null ? "" : attendance.get(i).getCheckOutTimeHHMM());
	  ...
	  public String checkInEmployee(String jsonObject) {
		  ...
		  try {
			  ...
			  String checkInTime = dao.checkInEmployee(inObj.getString("hotelId"), inObj.getString("employeeId"), inObj.getInt("shift"));
	  ...
	  public String markAbsent(String jsonObject) {
		  ...
		  try {
			  ...
			  outObj.put("status", dao.markAbsent(inObj.getString("hotelId"), inObj.getString("employeeId"), inObj.getInt("shift")));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  public String markExcused(String jsonObject) {
		  ...
		  try {
			  ...
			  outObj.put("status", dao.markExcused(inObj.getString("hotelId"), inObj.getString("employeeId"), inObj.getString("reason"), inObj.getInt("shift")));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  private void printKOT(String hotelId, String orderId) {
		  ...
		  if(beverageItems.size() > 0) {
			  defineKOT(beverageItems, dao, "BEVERAGE ORDER TAKING", Configurator.getBeveragePrinter(), hotelId, order, copies, false);
		  }
	      if(kitchenItems.size() > 0) {
	    	  	  defineKOT(kitchenItems, dao, "KITCHEN ORDER TAKING", Configurator.getKitchenPrinter(), hotelId, order, copies, false);
	      }
	      if(barItems.size() > 0) {
	    	  	  if(!hotelId.equals("sa0001"))
	    	  		  defineKOT(barItems, dao, "BAR ORDER TAKING", Configurator.getBarPrinter(), hotelId, order, copies, false);
	      }
	  ...
	  private void defineKOT(ArrayList<OrderItem> items, AccessManager dao, String title, String printerStation, 
		  ...
		  out += "<tr><td>"+items.get(0).getSubOrderDate().substring(8, 10)+items.get(0).getSubOrderDate().substring(4, 7)
			+ "/"+items.get(0).getSubOrderDate().substring(0, 4)+"</td>"
			+ "<td>"+items.get(0).getSubOrderDate().substring(11, 16)+"</td>";
		  String name = "";
		  if(table!=null)
		  ...
		  for(int i = 0; i < items.size(); i++) {
			  ...
			  for(int j=1; j<=quantity;j++) {
				  specifications = dao.getOrderedSpecification(hotelId, items.get(i).getOrderId(), items.get(i).getMenuId(),
						  items.get(i).getSubOrderId(), j);
				  if(isCanceled)
					  addOns = dao.getCanceledOrderedAddOns(hotelId, items.get(i).getOrderId(), items.get(i).getSubOrderId(), items.get(i).getMenuId(), j);
				  else
					  addOns = dao.getOrderedAddOns(hotelId, items.get(i).getOrderId(), items.get(i).getSubOrderId(), items.get(i).getMenuId(), j);
				  ...
				  if(specifications.size()==0 && addOns.size()==0) {
					  ...
					  if(hotelId.equals("ka0001")) {
						  out += "<tr class='marg-td-small'><td class='pull-left' "+fontWeight+">" + currentQuantity + "</td>"
							  + "<td class='pull-left'>" + formatTitle(items.get(i).getTitle(), true)
							  + "</td><td></td></tr>";
					  }else {
				  ...
				  if(!isCanceled) {
					  ...
					  out += "</td><td class='pull-right'>" + spec
							  + "</td></tr>";
				  }
				  ...
				  for(int k=0; k<addOns.size(); k++){
					  ...
					  if(hotelId.equals("ka0001"))
						  out += "<tr class='marg-td-small'><td class='pull-left addOn-td' "+fontWeight+">" + addOns.get(k).getQty() + "</td><"
							  + "<td class='addOn-td pull-left' style='padding-left:50px !important'>+ " + formatTitle(addOns.get(k).getName(), true);
					  else
						  out += "<tr class='marg-td'><td class='pull-left addOn-td' "+fontWeight+">" + addOns.get(k).getQty() + "</td><"
							  + "<td class='addOn-td pull-left' style='padding-left:50px !important'>+ " + formatTitle(addOns.get(k).getName(), false);
					  if(!isCanceled)
						  out += "</td><td class='addOn-td'>"
								  + "</td><td></td>/tr>";
				  }
				  spec = "";
				  currentQuantity=0;
		  ...
		  out = "<html><head><style>.table-condensed>thead>tr>th, .table-condensed>tbody>tr>th,"
				  ...
				  +	"padding: 5px;} .mb0 {margin-bottom: 0px;} .ml25 {margin-left: 25px;} .ml2 {margin-left: 20px;} "
				  ...
				  +	" h3, .h3 {font-size: 18px;} .marg-td{ margin-top:0px; margin-bottom:2px; font-size:12px; font-weight: bold;}"
				  + " .marg-td-small{ margin-top:0px; margin-bottom:0px; font-size:11px; font-weight: bold;}"
				  +	"</style></head><body style='width: 377px; font-family:"+Configurator.getKOTFont()+";'>" + out + "</body></html>";
	  ...
	  @Path("/v1/getOrder")
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getOrder(@QueryParam("hotelId") String hotelId, @QueryParam("orderId") String orderId, @QueryParam("showConsolidated") Boolean showConsolidated,
		  ...
		  try {
			  ...
			  if(order.getState() == AccessManager.ORDER_STATE_OFFKDS || order.getState() == AccessManager.ORDER_STATE_SERVICE) {
			  ...
			  for (int i=0;i<orderItems.size(); i++) {
				  ...
				  if(orderItems.get(i).getItemId() == 0) {
					  for (int j=1;j<=orderItems.get(i).getQty(); j++) {
						  addOns = dao.getReturnedAddOns(hotelId, orderId, orderItems.get(i).getSubOrderId(), orderItems.get(i).getMenuId(), j);
						  for (int k=0;k<addOns.size(); k++) {
							addOnDetails = new JSONObject();
					  ...
					  addOns = dao.getReturnedAddOns(hotelId, orderId, orderItems.get(i).getSubOrderId(), orderItems.get(i).getMenuId(), orderItems.get(i).getItemId());
					  for (int k=0;k<addOns.size(); k++) {
						addOnDetails = new JSONObject();
						addOnDetails.put("name", addOns.get(k).getName());
						addOnDetails.put("itemId", addOns.get(k).getItemId());
	  ...
	  public String getAllOrders(@QueryParam("hotelId") String hotelId, @QueryParam("dateFilter") String dateFilter
		  ...
		  try {
			  for(int i =0; i<order.size(); i++){
				  ...
				  itemDetails.put("tableId", order.get(i).getTableId().length() == 3||order.get(i).getTableId().length() == 2 ? order.get(i).getTableId().replace(",", ""): order.get(i).getTableId());
		  ...
		  try {
			  ...
			  for(int i=0; i<orderItems.length(); i++) {
				  ...
				  if(type.equals("return")) {
				  		...
				  		dao.updateOrderItemLog(hotelId, orderId, orderItems.getJSONObject(i).getString("subOrderId"),
					  		orderItems.getJSONObject(i).getString("menuId"), orderItems.getJSONObject(i).getString("reason"), 
					  		type, returnQuantity, orderItems.getJSONObject(i).getInt("rate"), 
						...
						outObj.put("status", true);
				  		...
				  		dao.updateOrderItemLog(hotelId, orderId, orderItems.getJSONObject(i).getString("subOrderId"),
				  				orderItems.getJSONObject(i).getString("menuId"), orderItems.getJSONObject(i).getString("reason"), 
				  				type, orderItems.getJSONObject(i).getInt("qty"), orderItems.getJSONObject(i).getInt("rate"), 
				  				orderItems.getJSONObject(i).getInt("qty"));
				  }else{
					  	quantity = 0;
				  		dao.updateOrderItemLog(hotelId, orderId, orderItems.getJSONObject(i).getString("subOrderId"),
				  ...
				  if(isRemoved){
					  ...
					  for (OrderAddOn orderAddOn : addOns) {
						  dao.updateOrderAddOnLog(hotelId, orderId, orderItems.getJSONObject(i).getString("subOrderId"), 
								  orderItems.getJSONObject(i).getString("menuId"), orderAddOn.getItemId(), "void", orderAddOn.getQty(), orderAddOn.getRate(), orderAddOn.getAddOnId());
						  ...
						  dao.updateFoodBillAddOnReturn(hotelId, orderId, orderItems.getJSONObject(i).getString("subOrderId"), 
								  orderItems.getJSONObject(i).getString("menuId"), orderAddOn);
					  }
					  outObj.put("status", true);
					  outObj.put("isTaxable", dao.isTaxableMenuItem(hotelId, orderItems.getJSONObject(i).getString("menuId")));
				  }
				  if(type.equals("return")) {
						...
						String text = "<div style='width:350px; ' class='alert alert-warning'><h3>An Item has been returned from Order. Bill No. " 
							...
							+ "</div><div>Order punched by: " + orderId.split(":")[0]
						  	+ "</div><div>Item Returned by : " + inObj.getString("userId")
							+ "</div><div>Reason: " + orderItems.getJSONObject(i).getString("reason") + "</div></div>";
						SendVoidReturnEmail(hotelId, subject, text, null, orderId);
				  }
			  }
			  if(beverageItems.size() > 0) {
				  defineKOT(beverageItems, dao, "BEVERAGE ORDER TAKING", Configurator.getBeveragePrinter(), hotelId, order, 1, true);
			  }
		      if(kitchenItems.size() > 0) {
		    	  	  defineKOT(kitchenItems, dao, "KITCHEN ORDER TAKING", Configurator.getKitchenPrinter(), hotelId, order, 1, true);
		    	  }
		      if(barItems.size() > 0) {
	  ...
	  public String voidOrder(String jsonObject) {
		  ...
		  try {
			  ...
			  if(dao.voidOrder(inObj.getString("hotelId"), orderId, inObj.getString("reason")
					  , inObj.getString("authId"))){
				  outObj.put("status", true);
				  String subject = "Bill No. " + billNo + " marked void!";
				  String text = "<div style='width:300px; '><div class='alert alert-warning'><b>Bill No. " + billNo + " </b>has been marked void.</h3><p> Details as follows:</p>"
				  		+ "<div>Time: " + LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_TIME)
						+ "</div><div>Order punched by: " + orderId.split(":")[0]
				  		+ "</div><div>Authorizer: " + inObj.getString("authId")
	  ...
	  public String complimentaryOrder(String jsonObject) {
		  ...
		  try {
			  ...
			  if(dao.complimentaryOrder(inObj.getString("hotelId"), orderId, inObj.getString("authId"))){
				  ...
				  String text = "<div style='width:300px; '><div class='alert alert-warning'><b>Bill No. " + billNo + " </b>has been marked complimentary.</h3><p> Details as follows:</p>"
				  		+ "<div>Time: " + LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_TIME)
						+ "</div><div>Order punched by: " + orderId.split(":")[0]
				  		+ "</div><div>Authorizer: " + inObj.getString("authId") + "</div></div>";
				  SendVoidReturnEmail(inObj.getString("hotelId"), subject, text, null, inObj.getString("orderId"));
			  }
		  }
		  catch(Exception e) {
		  ...
		  try {
			  ...
			  for(int i=0; i<compItems.length(); i++) {
				  if(compItems.getJSONObject(i).getBoolean("status"))
					  dao.complimentaryItem(inObj.getString("hotelId"), orderId, compItems.getJSONObject(i).getString("menuId"), inObj.getString("authId")
						  , compItems.getJSONObject(i).getString("subOrderId"), compItems.getJSONObject(i).getInt("rate"), compItems.getJSONObject(i).getInt("qty"));
				  JSONArray compAddons = compItems.getJSONObject(i).getJSONArray("compAddOns");
				  
				  for(int j=0; j<compAddons.length(); j++) {
					  dao.complimentaryAddOn(inObj.getString("hotelId"), orderId, compAddons.getJSONObject(j).getInt("addOnId"), 
							  inObj.getString("authId"), compItems.getJSONObject(i).getString("menuId"), 
							  compItems.getJSONObject(i).getString("subOrderId"), compAddons.getJSONObject(j).getInt("itemId"),
							  compAddons.getJSONObject(j).getInt("rate"), compAddons.getJSONObject(j).getInt("qty"));
				  }
			  }
		  }
		  catch(Exception e) {
	  ...
	  public String editOrder3(String jsonObject) {
		  ...
		  try {
			  ...
			  for (int i=0;i<newItems.length();i++) {
				  ...
				  for (int j=0;j<addOns.length();j++) {
					  Boolean addedAddOn = dao.addOrderAddon(hotelId, orderId, menuId, 
						  addOns.getJSONObject(j).getInt("qty"), 
						  addOns.getJSONObject(j).getInt("id"), 
						  subOrderId,
				  ...
				  for (int j=0;j<specifications.length();j++) {
					  if(specifications.getJSONObject(j).getInt("itemId")==101) {
						  ...
						  addedSpec  = dao.addOrderSpecification(hotelId, orderId,
								  subOrderId, menuId,
			  ...
			  for (int i=0;i<changeStateItems.length();i++) {
				  if(!dao.changeOrderStatus(hotelId, orderId, changeStateItems.getJSONObject(i).getString("subOrderId"),changeStateItems.getJSONObject(i).getString("menuId")))
				  {
					  dao.rollbackTransaction();
		  ...
		  try {
			  ...
			  for (int i=0;i<newItems.length();i++) {
				  ...
				  subOrder = dao.newSubOrder(inObj.getString("hotelId"),
						  ...
						  newItems.getJSONObject(i).getInt("qty"), newItems.getJSONObject(i).getString("spec"),
						  subOrderId, inObj.has("userId")?inObj.getString("userId"):"", 0);
				  if (subOrder==null || subOrder.getInt("status")==-1) {
					  dao.rollbackTransaction();
					  if (subOrder != null) {
						  outObj.put("message", subOrder.get("message"));
				  ...
				  JSONArray addOns = newItems.getJSONObject(i).getJSONArray("addOnArr");
				  for (int j=0;j<addOns.length();j++) {
					  Boolean addedAddOn = dao.addOrderAddon(inObj.getString("hotelId"),
						  inObj.getString("orderId"), 
						  newItems.getJSONObject(i).getString("menuId"), 
						  addOns.getJSONObject(j).getInt("qty"), 
						  addOns.getJSONObject(j).getInt("id"), 
			  ...
			  dao.addPayment(inObj.getString("hotelId"), inObj.getString("orderId"), dao.getServiceDate(inObj.getString("hotelId")),
					  inObj.getDouble("foodBill"), inObj.getDouble("barBill"), inObj.getDouble("discount"), inObj.getDouble("total"), 
					  inObj.getDouble("sc"), inObj.getDouble("gst"), inObj.getDouble("tip"), inObj.getDouble("cashPayment"), 
	  ...
	  public String modifyCustomer(String jsonObject) {
		  ...
		  try {
			  JSONObject inObj = new JSONObject(jsonObject);
			  AccessManager dao = new AccessManager(false);
			  outObj.put("status", dao.modifyCustomer(inObj.getString("hotelId"),
					  inObj.getString("name"),
					  inObj.getString("phone"),
					  inObj.getString("birthdate"),
					  inObj.getString("anniversary"),
					  inObj.getString("allergyInfo"),
					  "", inObj.getBoolean("wantsPromotion")));
		  } catch(Exception e) {
			  e.printStackTrace();
		  }
		  return outObj.toString();
	  ...
	  public String newHomeDeliveryOrder(String jsonObject) {
		  ...
		  try {
			  ...
			  outObj = dao.newHomeDeliveryOrder(inObj.getString("hotelId"), inObj.getString("userId"), 
					  inObj.getString("customer"),inObj.getString("mobile"),inObj.getString("address"),inObj.getString("allergyInfo"));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String hasCheckedOut(@QueryParam("hotelId") String hotelId) {
		  JSONObject outObj = new JSONObject();
		  AccessManager dao = new AccessManager(false);
		  try {
			  outObj.put("status", false);
	  ...
	  public String addDiscount(String jsonObject) {
		  ...
		  try {
			  ...
			  outObj.put("status",dao.addDiscount(inObj.getString("hotelId"),inObj.getString("name"), inObj.getString("description"), inObj.getInt("type"), 
					  ...
					  inObj.getString("validCollections")));
			  }
		  }
		  catch(Exception e) {
			  e.printStackTrace();
	  ...
	  public String editDiscount(String jsonObject) {
		  ...
		  try {
			  ...
			  outObj.put("status",dao.editDiscount(inObj.getString("hotelId"),inObj.getString("name"), inObj.getString("description"), inObj.getInt("type"), 
					  inObj.getInt("value"), inObj.getString("startDate"), inObj.getString("expiryDate"), inObj.getString("usageLimit"),
					  inObj.has("validCollections")?inObj.getString("validCollections"):""));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	...
	private String getPreviousDateString(int day) {
	  ...
	  public String addPayment(String jsonObject) {
		  ...
		  try {
			  ...
			  boolean status = dao.addPayment(hotelId, orderId, inObj.getString("orderDate"), 
					  inObj.getDouble("foodBill"), inObj.getDouble("barBill"), inObj.getDouble("discount"), inObj.getDouble("total"), 
					  inObj.getDouble("sc"), inObj.getDouble("gst"), inObj.getDouble("tip"), inObj.getDouble("cashPayment"), 
					  inObj.getDouble("cardPayment"), inObj.getString("discountName"), inObj.getString("cardType"), inObj.getDouble("complimentary"));
			  ...
			  if(status){
				  ...
				  if(hotel.getHotelType().equals("PREPAID") && hotel.getKDSType().equals("KDS"))
					  ...
					  dao.markPaymentComplete(hotelId, orderId);
			  ...
			  if(!inObj.getString("discountName").equals("")){
				  ...
				  if(!discount.getUsageLimit().equals("Unlimited")){
					  ...
					  dao.updateUsageLimit(hotelId, discount.getName(), usageLimit);
				  }
			  }
	  ...
	  public String updateStock(String jsonObject) {
		  ...
		  try { 
			  ...
			  outObj.put("status",dao.updateStock(inObj.getString("hotelId"), inObj.getString("sku"),
					  newQuantity, inObj.getDouble("quantity"), inObj.getDouble("ratePerUnit"), doe));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getLunchDinnerSalesReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate,
			  @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
		  ...
		  try {
			  ArrayList<LunchDinnerSalesReport> reportItems0 = dao.getLunchDinnerSalesReport(hotelId, startDate, endDate, 0);
			  ArrayList<LunchDinnerSalesReport> reportItems1 = dao.getLunchDinnerSalesReport(hotelId, startDate, endDate, 1);
			  ArrayList<LunchDinnerSalesReport> reportItems2 = dao.getLunchDinnerSalesReport(hotelId, startDate, endDate, 2);
			  ...
			  itemDetails3 = new JSONObject();
			  {
				  ...
				  itemDetails1.put("foodPPax", Math.round(reportItems0.get(0).getFoodBill()/reportItems0.get(0).getPax()));
				  itemDetails1.put("barPPax",  Math.round(reportItems0.get(0).getBarBill()/reportItems0.get(0).getPax()));
				  ...
				  itemDetails2.put("foodPPax",  Math.round(reportItems1.get(0).getFoodBill()/reportItems0.get(0).getPax()));
				  itemDetails2.put("barPPax",  Math.round(reportItems1.get(0).getBarBill()/reportItems0.get(0).getPax()));
				  ...
				  itemDetails3.put("foodPPax",  Math.round(reportItems2.get(0).getFoodBill()/reportItems0.get(0).getPax()));
				  itemDetails3.put("barPPax",  Math.round(reportItems2.get(0).getBarBill()/reportItems0.get(0).getPax()));
	  ...
	  public String getTotalSalesForService(@QueryParam("hotelId") String hotelId, @QueryParam("serviceDate") String serviceDate,
		  ...
		  try {
			  ...
			  temp = Math.round(reportItems.getGrossTotal()*100);
			  outObj.put("grossTotal", temp/100);
			  ...
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.MASTERCARD.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  arrObj.put("name", CardType.MAESTRO.toString());
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.MAESTRO.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  ...
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.RUPAY.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  arrObj.put("name", CardType.MSWIPE.toString());
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.MSWIPE.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  arrObj.put("name", CardType.OTHERS.toString());
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.OTHERS.toString()));
			  arr.put(arrObj);
			  ...
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.ZOMATO.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  arrObj.put("name", CardType.SWIGGY.toString());
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.SWIGGY.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  arrObj.put("name", CardType.MAGIC_PIN.toString());
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.MAGIC_PIN.toString()));
			  arr.put(arrObj);
			  arrObj = new JSONObject();
			  arrObj.put("name", CardType.PAYTM.toString());
			  arrObj.put("amount", dao.getCardPaymentByType(hotelId, serviceDate, serviceType, CardType.PAYTM.toString()));
			  arr.put(arrObj);
			  outObj.put("apps", arr);
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getDailyDiscountReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate,
			  @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getCollectionWiseReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate,
			  @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getDailyOperationReport1(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate,
			  @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getDailyOperationReport2(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate,
			  @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
		  ...
		  ArrayList<DailyOperationReport> reportItems5 = dao.getDailyOperationReport5(hotelId,startDate, endDate);
	   	  //voids
		  //ArrayList<DailyOperationReport> reportItems6 = dao.getDailyOperationReport6(hotelId,startDate, endDate);
		  //returns
		  //ArrayList<DailyOperationReport> reportItems7 = dao.getDailyOperationReport7(hotelId,startDate, endDate);
		  try {
	...
	private void generateShortForm() {
	  ...
	  public String addRevenue(String jsonObject) {
		  ...
		  try {
			  ...
			  boolean status = dao.addRevenue(inObj.getString("hotelId"), 
					  ...
					  inObj.getDouble("card"), inObj.getDouble("app"), inObj.getDouble("total"), inObj.getDouble("visa"), 
					  ...
					  inObj.getDouble("swiggy"), inObj.getDouble("magicPin"), inObj.getDouble("payTm"), inObj.getDouble("difference"), 
	  ...
	  public String getOrdersOfOneCustomer(@QueryParam("hotelId") String hotelId, @QueryParam("mobileNo") String mobileNo) { //Jason
		  AccessManager dao = new AccessManager(false);
	  ...
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getItemwiseReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate, @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
		  JSONObject itemDetails = null;
	  ...
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getLiquorReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate, @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
		  JSONObject itemDetails = null;
	  ...
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getIncentiveReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate, @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
		  JSONObject itemDetails = null;
		  ...
		  try {
			  for (int i=0;i<captains.size(); i++) {
				  itemDetails = new JSONObject();
				  reportItemsFood = dao.getIncentiveForEmployee(hotelId, captains.get(i).getEntity(), false, startDate, endDate);
				  ...
				  reportItemsBar = dao.getIncentiveForEmployee(hotelId, captains.get(i).getEntity(), true, startDate, endDate);
	  ...
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getItemwiseIncentiveReport(@QueryParam("hotelId") String hotelId, @QueryParam("startDate") String startDate, @QueryParam("endDate") String endDate) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray itemsArr = new JSONArray();
		  ...
		  try {
			  ...
			  for (int i=0;i<captains.size(); i++) {
				  ...
				  itemsArr = new JSONArray();
				  reportItems = dao.getItemwiseIncentiveReport(hotelId, captains.get(i).getEntity(), startDate, endDate);
	  ...
	  public String moveItem(String jsonObject) {
		  ...
		  try {
			  ...
			  boolean status = dao.moveItem(inObj.getString("hotelId"), inObj.getInt("oldTableNumber"), 
	  ...
	  public String updateServerStatus(String jsonObject) {
		  ...
		  try {
			  inObj = new JSONObject(jsonObject);
			  //dao.updateServerStatus(inObj.getString("hotelId"), inObj.getBoolean("clearLogs"));
...
//-----------------------------------------------------------------------Loyalty
	  ...
	  public String addLoyalty(String jsonObject) {
		  ...
		  try {
			  outObj.put("status", false);
			  inObj = new JSONObject(jsonObject);
			  if(dao.addLoyaltyOffer(inObj.getString("hotelId"), inObj.getString("name"), inObj.getString("description"), 
					  inObj.getInt("points"), inObj.getInt("offerType"), inObj.getString("offer"))){
				  outObj.put("status", true);
			  }else
				  outObj.put("error", "This offer already exists Exists");
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  @Produces(MediaType.APPLICATION_JSON)
	  @Consumes(MediaType.APPLICATION_JSON)
	  public String editLoyalty(String jsonObject) {
		  JSONObject inObj = null;
		  JSONObject outObj = new JSONObject();
		  AccessManager dao = new AccessManager(false);
		  try {
	  ...
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String getAllLoyaltyOffers(@QueryParam("hotelId") String hotelId) {
		  AccessManager dao = new AccessManager(false);
		  JSONArray loyaltyOfferArr = new JSONArray();
		  JSONObject outObj = new JSONObject();
		  JSONObject loyaltyDetails = null;
		  ArrayList<LoyaltyOffer> loyaltyOffers = dao.getAllLoyaltyOffers(hotelId);
		  try {
			  for (int i=0;i<loyaltyOffers.size(); i++) {
				  loyaltyDetails = new JSONObject();
				  loyaltyDetails.put("status", true);
				  loyaltyDetails.put("name", loyaltyOffers.get(i).getName());
				  loyaltyDetails.put("description", loyaltyOffers.get(i).getDescription());
				  loyaltyDetails.put("points", loyaltyOffers.get(i).getPoints());
				  loyaltyDetails.put("offer", loyaltyOffers.get(i).getOffer());
				  loyaltyDetails.put("offerType", loyaltyOffers.get(i).getOfferType());
				  loyaltyDetails.put("count", loyaltyOffers.get(i).getCount());
				  loyaltyOfferArr.put(loyaltyDetails);
			  }
			  outObj.put("discounts", loyaltyOfferArr);
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
	  ...
	  public String getLoyaltyOffer(@QueryParam("hotelId") String hotelId, @QueryParam("id") int id) {
		  ...
		  try {
			  	  ...
			  	  if(loyalty != null) {
				   	  ...
				   	  outObj.put("description", loyalty.getDescription());
				   	  outObj.put("points", loyalty.getPoints());
				   	  outObj.put("offer", loyalty.getOffer());
				   	  outObj.put("offerType", loyalty.getOfferType());
				   	  outObj.put("count", loyalty.getCount());
			  	  }
		  }
		  catch(Exception e) {
			  e.printStackTrace();
	  ...
	  @POST
	  @Path("/v1/deleteLoyaltyOffer")
	  @Consumes(MediaType.APPLICATION_JSON)
	  @Produces(MediaType.APPLICATION_JSON)
	  public String deleteLoyaltyOffer(String jsonObject) {
		  JSONObject inObj = null;
		  JSONObject outObj = new JSONObject();
		  AccessManager dao = new AccessManager(false);
		  try {
			  outObj.put("status", false);
			  inObj = new JSONObject(jsonObject);
			  outObj.put("status",dao.deleteLoyaltyOffer(inObj.getString("hotelId"), inObj.getInt("id")));
		  }
		  catch(Exception e) {
			  e.printStackTrace();
		  }
		  ...
		  try {
			  ...
			  for (int i = 0; i < inArr.length(); i++) {
				  ...
				  dao.assignWaiterToTable(inObj.getString("hotelId"), 
						  waiterObj.has("waiter")?waiterObj.getString("waiter"):"", 
						  waiterObj.getInt("tableId"));
			  }
			  outObj.put("status", true);
		  }
		  catch(Exception e) {
