...
public class ReportManager extends AccessManager implements IReport{
	...
	public ArrayList<Expense> getExpenseReport(String hotelid, String startDate, String endDate, String filter) {
		...
		String sql = "SELECT *, "
				+ "REPLACE(REPLACE(REPLACE(Expenses.memo,CHAR(13),''),CHAR(10),''),',','|') AS description " // replaced ','
																										// with '|' AND
																										// '/n' with ' '
				+ "FROM Expenses WHERE hotelId = '" + hotelid + "' AND type != 'PAYIN'" 
				+ "AND serviceDate BETWEEN '" + startDate + "' AND '" + appendEndDate(endDate) + "'" + filterText;
	...
	public ArrayList<GrossSaleReport> getGrossSalesReport(String hotelId, String startDate, String endDate, boolean visible) {
		...
		String sql = "SELECT ROUND(SUM(payment.foodBill+payment.barBill+payment.gst+payment.VATBAR+payment.serviceCharge+payment.deliveryCharge+payment.packagingCharge)*100)/100 as grossTotal, "
				...
				+ "ROUND((SUM(payment.total)-(Select ifnull(SUM(Expenses.amount),0) From Expenses Where (expenses.type!='CASH_LIFT' OR expenses.type!='PAYIN') AND expenses.serviceDate BETWEEN '"
				+ startDate + "' AND '" + endDate + "' AND expenses.hotelId='" + hotelId + "'))*100)/100 as netSales, "
				+ "ROUND((Select ifnull(SUM(Expenses.amount),0) From Expenses Where (expenses.type!='CASH_LIFT' OR expenses.type!='PAYIN') AND expenses.serviceDate BETWEEN '"
				+ startDate + "' AND '" + endDate + "' AND expenses.hotelId='" + hotelId
				+ "')*100)/100 as grossExpenses, "
				+ "ROUND((Select ifnull(SUM(Expenses.amount),0) From Expenses Where expenses.type=='PAYIN' AND expenses.serviceDate BETWEEN '"
				+ startDate + "' AND '" + endDate + "' AND expenses.hotelId='" + hotelId
				...
				+ "AND Orders.orderDate BETWEEN '" + startDate + "' AND '" + endDate + "' AND Payment.cardType != 'VOID'";
		if(!visible) 
 ...
 GROUP BY sku
	...
	public ArrayList<ReturnedItemsReport> getReturnedItemsReport(String hotelId, String startDate, String endDate, boolean visible) {
		...
		if(endDate.equals("") || endDate.equals(startDate)) {
			sql += "AND OrderItemlog.dateTime LIKE '" + startDate + "%' ";
		}else {
			sql += "AND OrderItemlog.dateTime BETWEEN '" + startDate + "%' AND '" + endDate + "%' ";
		}
		if(!visible)
	...
	public Report getTotalSalesForService(String hotelId, String serviceDate, String serviceType, String section, boolean visible) {
		...
		String sql = "SELECT ROUND(SUM(Payment.foodBill+Payment.barBill+gst+VATBAR+serviceCharge)*100)/100 AS grossTotal, "
				+ "ROUND(SUM(Payment.foodBill)*100)/100 AS foodBill, "
				+ "ROUND(SUM(Payment.barBill)*100)/100 AS barBill, "
	...
	public BigDecimal getTotalWalletPayment(String hotelId, String serviceDate, String serviceType) {
		...
		String sql = "SELECT SUM(Payment.walletPayment) as entityId FROM Payment,Orders "
				+ "WHERE Orders.orderDate = '" + serviceDate + "' AND Orders.serviceType = '" + serviceType + "' "
				+ "AND Orders.hotelId = '" + hotelId + "' AND Payment.orderId = Orders.orderId;";
	...
	public BigDecimal getTotalPromotionalCash(String hotelId, String serviceDate, String serviceType) {
		...
		String sql = "SELECT SUM(Payment.promotionalCash) as entityId FROM Payment,Orders "
				+ "WHERE Orders.orderDate = '" + serviceDate + "' AND Orders.serviceType = '" + serviceType + "' "
				+ "AND Orders.hotelId = '" + hotelId + "' AND Payment.orderId = Orders.orderId;";
	...
	public BigDecimal getTotalCreditAmount(String hotelId, String serviceDate, String serviceType) {
		...
		String sql = "SELECT SUM(Payment.creditAmount) as entityId FROM Payment,Orders "
				+ "WHERE Orders.orderDate = '" + serviceDate + "' AND Orders.serviceType = '" + serviceType + "' "
				+ "AND Orders.hotelId = '" + hotelId + "' AND Payment.orderId = Orders.orderId;";
